 ...
 *
 * Usage: npm run autoSetup (or runs automatically after npm install)
 */
...
function readExistingClientConfig(client: MCPClient): Record<string, unknown> {
  ...
  const parsed = Effect.runSync(
    Effect.either(
      Effect.try({
        try: () => readRecord(JSON.parse(readFileSync(client.configPath, 'utf-8'))),
        catch: (error) => error,
      }),
    ),
  );
...
function updateClientConfig(client: MCPClient, serverConfig: MCPServerConfig): boolean {
    ...
    Effect.either(
      Effect.try({
        try: () => {
          ...
          const config = readExistingClientConfig(client);
  ...
  if (Either.isLeft(updated)) {
    printError(`Failed to update ${client.name} config: ${getErrorMessage(updated.left)}`);
    return false;
  }
